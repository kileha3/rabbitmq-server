...
query_node_props(Nodes) when Nodes =/= [] ->
    ...
    %% globally registered controllers end up in the same "namespace" and
    %% `global' will kill all but one of them.
    %%
    %% By using a hidden node, we ask Erlang not to connect everyone
    %% automatically.
